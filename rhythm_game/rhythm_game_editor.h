...
void data_hyouji(int, int, MUSIC_DATA* data, char);
void editor(char note_gamen[ARRAY_GYO][7], FILE* fp, char editor_mode, char name[80])
{
	...
	if (editor_mode == OVERWRITE_MODE || editor_mode == NEW_MODE)
	{
		...
		do
		{
			if (scanf("%d", &editor.tempo[ARRAY_GYO - 3]) != 1)
				while (getchar() != '\n');
			printf("\x1b[H\x1b[K");
		} while (editor.tempo <= 0);
		editor.tempo_now = editor.tempo[ARRAY_GYO - 3];
		fprintf(fp, "%d %s\n", editor.tempo[ARRAY_GYO - 3], editor.note[ARRAY_GYO - 3]);
	}
	else
	{
		...
		while (fscanf(fp, "%d %s", &editor.tempo[0], editor.note[0]) != EOF)
		{
			if (editor.tempo[0] != 0) editor.tempo_now = editor.tempo[0];
			descent(ARRAY_GYO, ARRAY_GYO - 1, &editor, fp_work);
			if (editor_note_gyo_max != ARRAY_GYO) editor_note_gyo_max++;
	...
	while (1)
	{
		printf("\x1b[H");
		editor_gyo = 0;
		data_hyouji(editor_gyo, editor_tempo_gyo_max, &editor, PROCESS_CHOICE);
		do	//選択メニュー1
		{
			printf("キー指定:");
			if (scanf(" %c", &kye) != 1)
			...
			case 'g':
				...
				break;
			case 't':
				if (editor_gyo <= 2 && editor_tempo_gyo_max >= ARRAY_GYO - 2 && editor_gyo + (ARRAY_GYO - 2) <= editor_tempo_gyo_max)	//テンポ(ノーツ速度)変更
				{
					do
					{
						printf("ノーツ速度(BPM):");
		...
		while (1)	//選択メニュー2
		{
			data_hyouji(editor_gyo, editor_tempo_gyo_max, &editor, NOTE_CHOICE);
			do
			{
				printf("ノーツ選択:");
			...
			data_hyouji(editor_gyo, editor_tempo_gyo_max, &editor, KYE_CHOICE);
			do
			{
				printf("キー指定:");
				if (scanf(" %c", &kye) != 1)
		...
		if (editor_gyo == 0)	//行降下処理兼データ移動
		{
			...
			if (editor_note_gyo_max != ARRAY_GYO) editor_note_gyo_max++;	//ノーツデータ編集行の上限増加
		}
	}
...
void data_hyouji(int gyo, int gyo_max, MUSIC_DATA* data, char id)
{
	int i, j;
	int bpm_valie;
	printf("\x1b[H");
	...
	case PROCESS_CHOICE:	//行程1
		...
		printf("             ┃ キー選択              ┃\n             ┃  a s d j k l          ┃\n");
		printf("             ┃ その他                ┃\n             ┃  g:行変更             ┃\n");
		if (gyo <= 2 && gyo_max >= ARRAY_GYO - 2 && gyo + (ARRAY_GYO - 2) <= gyo_max) printf("             ┃  t:ノーツ速度変更     ┃\n");
		else printf("             ┃                       ┃\n");
		printf("             ┃  q:譜面制作終了       ┃\n");
		printf("             ┗━━━━━━━━━━━━━━━━━━━━━━━┛\n");break;
...
void descent(int gyo_max, int tempo_gyo_max, MUSIC_DATA* data, FILE* fp)
{
	...
	if (tempo_gyo_max == ARRAY_GYO - 1) fprintf(fp, "%d %s\n", data->tempo[ARRAY_GYO - 1], data->note[ARRAY_GYO - 1]);
