...
void main()
{
	...
	if ((fp = file_open("system_data.txt", "r")) == NULL)
	{
		//ファイル無い時にファイル作成
		if ((fp = file_open("system_data.txt", "w")) == NULL)
		{
	...
	while (1)
	{
		...
		{
			printf(GAMEN_CLEAR);
			if ((fp = file_open("gakkyoku.txt", "r")) == NULL || (fp_result = file_open("rhythm_game_result.txt", "r")) == NULL)
			{
			...
			fclose(fp);
			fclose(fp_result);
			if (j != SENTAKU_END && (k == EDITOR_KYE && i != j || k == PLARY_KYE))
			{
				if ((fp = file_open("gakkyoku.txt", "r")) == NULL)
				{
		...
		if (j != SENTAKU_END)
		{
			...
			else if (k == SYSTEM_KYE)	//設定
			{
				while (1)
				{
					...
					if (k == FPS_EDITOR)
					{
						...
						do
						{
							printf("選択(現在:%dfps):", system_data.fps_valie);
							if (scanf("%c", &k) != 1)
								while (getchar() != '\n');
							printf("\x1b[4H\x1b[K");
						} while (k != '1' && k != '2');
						...
						while (1)
						{
							...
							do
							{
								...
								if (scanf("%c", &k) != 1)
									while (getchar() != '\n');
								printf("\x1b[9H\x1b[k");
							} while (k < NOTE_SPEED_TEST || k>'5');
							while (getchar() != '\n');
							if (k == NOTE_SPEED_TEST)
							{
								printf(GAMEN_CLEAR);
								if ((fp = file_open("note_speed_test.txt", "r")) == NULL)
								{
			...
			else if (k == EDITOR_KYE)	//創作モード
			{
				if (i == j)
				{
					while (1)
					{
						if ((fp_work = file_open("gakkyoku.txt", "a")) == NULL)
						{
						...
						do
						{
							...
							strcat(name, ".txt");
						} while ((fp = file_open(name, "w")) == NULL);
					...
					k = NEW_MODE;
					if ((fp_work = file_open("rhythm_game_result.txt", "a")) == NULL)
					{
...
void gamen(SYSTEM* system_valie)
{
	...
	//game system main
	for (i = 0;i < PERFECT;i++)	//初期化
		tempo[i] = 0;
	if (fscanf(fp, "%d %*d/%*d %*s", &tempo[PERFECT - 1]) != 1)	//初期BPM代入
	{
		printf("読み込みエラーが発生しました。"ERROR_BPM_DATA);
		fclose(fp); exit(1);
	}
	for (fps_count = 0, combo = 0, time_now = 0.0, tempo_sum = 0.0, hantei_cnt = 0;game_end != 0;)
	{
		...
		{
			if (fps_count == note_speed - 1)	//新しいノーツデータ
			{
				if (fscanf(fp, "%d %*[^ ] %s", &tempo[0], note[0]) == EOF) game_end--;
				fps_count = 0;
			}
			else	//倍率によって見ためのノーツを代入
			{
				tempo[0] = 0;
				for (i = 0;i < 6;i++)
				{
			...
			if (tempo[PERFECT-1] != 0) tempo_sum = (((double)60 / tempo[PERFECT-1] * (double)1000) / (double)4) / (double)note_speed;	//{(60÷BPM×msに変換)÷1拍のノーツの数}
		...
		if ((*hit >> i & 0x1) == 1)
		{
			for (j = GREAT_LATE;j >= MISS_FAST;j--)
			{
				if (note[j][i] == notenumber || note[j][i] == notenumber_link)
				{
					...
					else if (j == MISS_FAST|| m==1 && j == MISS_FAST + m)
					{
						combo = 0;
						hantei = 2;
						score.seido.miss++;
...
void result(char play_name[80])
{
	...
	printf("\x1b[?25h");
	//リザルト保存
	if ((fp_result = file_open("rhythm_game_result.txt", "r")) == NULL || (fp = file_open("sub.txt", "w")) == NULL)
	{
	...
	fclose(fp_result);
	fclose(fp);
	file_delete("rhythm_game_result.txt");
	if (neme_change("sub.txt", "rhythm_game_result.txt") != 0)
	{
