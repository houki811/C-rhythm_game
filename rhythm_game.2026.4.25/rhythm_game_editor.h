...
void data_hyouji(char,int, int, int, int, MUSIC_DATA* data, char);
int gyo_warp(int,char,char name[80],MUSIC_DATA* editor_data);
void gyo_save(int, MUSIC_DATA* data, FILE* fp);
int editor(char note_gamen[ARRAY_GYO][7], FILE* fp, char editor_mode, char name[80])
{
	FILE* fp_work;
	int i, j;
	...
	{
		if ((fp_work = file_open("editor_sub.txt", "w")) == NULL)
		{
			printf("ファイルオープンエラーが発生しました。"ERROR_CONTINUATION_MODE);exit(1);
		}
	...
	while (1)
	{
		printf("\x1b[H");
		editor_gyo = 0;
		data_hyouji(gyo_flg,editor_note_gyo_max, editor_gyo, gamen_gyo_max, editor_tempo_gyo_max, &editor, PROCESS_CHOICE);
		do	//選択メニュー1
		{
			printf("キー指定:");
			if (scanf(" %c", &kye) != 1)
			...
			case 'a':break;
			case 'g':	//行変更
				if (IF_DIFFERENT(gyo_flg , SELECTION_RESTRICTIONS))
				{
					do	//行変更
					{
						printf("行指定:");
					...
					if (editor_gyo > ARRAY_GYO - 1)
					{
						if (gyo_flg == PAST_RETURN || gyo_flg == PAST_STRAT)	//さらに過去に移動するとき
						{
							gyo_flg = MORE_PAST | SELECTION_RESTRICTIONS;
							if ((fp_work = file_open(name, "r")) == NULL)
							{
								printf("メインデータが参考できませんでした。"ERROR_MORE_PAST); exit(1);
							}
							...
							file_delete(name);
							if (neme_change("editor_sub.txt", name) != 0)
							{
								printf("読み込みエラーが発生しました。"ERROR_MORE_PAST_RENAME); exit(1);
							}
						}
						else	//画面外移動開始
						{
							gyo_flg = PAST_STRAT | SELECTION_RESTRICTIONS;
						...
						if ((fp = file_open("editor_sub.txt", "a")) == NULL)	//一時保存データ用ファイル
						{
							printf("ファイルが開けません。"ERROR_PAST); exit(1);
						}
					}
					else if (IF_EQUAL(gyo_flg, PAST_STRAT) && editor_gyo < 0)	//画面外への移動時 - 最新への移動 -
					{
						if ((fp_work = file_open(name, "r")) == NULL)
						{
							printf("メインデータが参考できません。"ERROR_PAST_FILE_OPEN_1); exit(1);
						}
						for (i = 0;fscanf(fp_work, "%*d %*d/%*d %*s") != EOF && i < gamen_gyo_max;i++);	//コピー始める行までスキップ
						...
						{
							gyo_flg = PAST_RETURN | SELECTION_RESTRICTIONS;
						}
						for (j = i;fscanf(fp_work, "%d %d/%d %s", &editor.tempo[0], &editor.measure_beat, &editor.standard_beat, editor.note[0]) != EOF;j++)	//EOF(データの最後まで)コピー
						{
							gamen_gyo_max++;
						...
						if (neme_change("editor_sub.txt", name) != 0)
						{
							printf("読み込みエラーが発生しました。"ERROR_PAST_RENAME); exit(1);
						}
						...
						if (IF_EQUAL(gyo_flg, PAST_END))
						{
							...
							if (neme_change("editor_sub.txt", name) != 0)
							{
								printf("読み込みエラーが発生しました。"ERROR_PAST_END_RENAME); exit(1);
							}
							if ((fp = file_open(name, "a")) == NULL)	//これから使用するファイルオープン
							{
								printf("新規ファイルが開けませんでした。"ERROR_PAST_END); exit(1);
							}
						}
						else
						{
							if ((fp = file_open("editor_sub.txt", "a")) == NULL)
							{
								printf("ファイルが開けません。"ERROR_PAST_RETURN); exit(1);
							}
						}
					}
					data_hyouji(gyo_flg, editor_note_gyo_max, editor_gyo, gamen_gyo_max, editor_tempo_gyo_max, &editor, PROCESS_CHOICE);
		...
		while (1)	//選択メニュー2
		{
			data_hyouji(gyo_flg,editor_note_gyo_max, editor_gyo, gamen_gyo_max, editor_tempo_gyo_max, &editor, NOTE_CHOICE);
			do
			{
				printf("ノーツ選択:");
			...
			data_hyouji(gyo_flg,editor_note_gyo_max, editor_gyo, gamen_gyo_max, editor_tempo_gyo_max, &editor, KYE_CHOICE);
			do
			{
				printf("キー指定:");
				if (scanf(" %c", &kye) != 1)
		...
		{	
			if (IF_EQUAL(gyo_flg , SELECTION_RESTRICTIONS))	//画面外移動時初期の操作制限解除
				gyo_flg ^= 0x8;
			gamen_gyo_max++;
			descent(gamen_gyo_max, editor_tempo_gyo_max, &editor, fp);
			if (editor_tempo_gyo_max!=ARRAY_GYO-1) editor_tempo_gyo_max++;	//BPM参照開始カウント
			if((gyo_flg>>1)==0x0) editor_note_gyo_max++;	//ノーツデータ編集行の上限増加
			if (editor_note_gyo_max != gamen_gyo_max)	//次行のデータ読み込み
			{
				if ((fp_work = file_open(name, "r")) == NULL)
				{
					printf("メインデータが参考できません。"ERROR_PAST_FILE_OPEN_2); exit(1);
				}
				for (i = 0;fscanf(fp_work, "%*d %*d/%*d %*s") != EOF && i < gamen_gyo_max;i++);
				if (fscanf(fp_work, "%d %d/%d %s", &editor.tempo[0], &editor.measure_beat, &editor.standard_beat, editor.note[0]) != 4)
				{
					printf("読み込みエラーが発生しました。"ERROR_PAST_FILE_DATA);
					fclose(fp_work); exit(1);
				}
				fclose(fp_work);
			}
			if (IF_EQUAL(gyo_flg , PAST_STRAT)&&editor_note_gyo_max==gamen_gyo_max)	//過去に戻っていた状態から最新の行まで来た時
			{
				...
				if (neme_change("editor_sub.txt", name) != 0)
				{
					printf("読み込みエラーが発生しました。"ERROR_PAST_GYOEQUAL_RENAME); exit(1);
				}
				if ((fp = file_open(name, "a")) == NULL)	//これから使用するファイルオープン
				{
					printf("ファイルが開けません。"ERROR_PAST_GYOEQUAL); exit(1);
				}
			}
		}
	}
	...
	if (editor_note_gyo_max!=gamen_gyo_max)
	{
		if ((fp_work = file_open(name, "r")) == NULL)
		{
			printf("メインデータが参考できません。"ERROR_EDITOR_END); exit(1);
		}
		for (i = 0;fscanf(fp_work, "%*d %*d/%*d %*s") != EOF && i < gamen_gyo_max;i++);
		...
		if (neme_change("editor_sub.txt", name) != 0)
		{
			printf("読み込みエラーが発生しました。"ERROR_EDITOR_END_RENAME); exit(1);
		}
		return 1;
	}
	else
...
void data_hyouji(char flg,int data_max, int gyo, int gyo_max, int tempo_gyo_max, MUSIC_DATA* data, char id)	//画面表示
{
	int i, j;
	int bpm_valie;
	int measure;
	...
	case PROCESS_CHOICE:	//行程1
		...
		printf("             ┏━━━━━━━━━━━━━━━━━━━━━━━┓\n");
		printf("             ┃ キー選択              ┃\n             ┃  a s d j k l          ┃\n");
		if (IF_EQUAL(flg , SELECTION_RESTRICTIONS))
		{
			for(i=0;i<2;i++)
				printf("             ┃                       ┃\n");
			printf("             ┃  t:ノーツ速度変更     ┃\n             ┃                       ┃\n");
...
{ 
	...
	if (IF_EQUAL(flg , PAST_END) || IF_EQUAL(flg , MORE_PAST) || IF_EQUAL(flg, PAST_STRAT))
		max--;
	...
	if ((fp_work = file_open(name, "r")) == NULL || (fp = file_open("editor_sub.txt", "w")) == NULL)
	{
		printf("ファイルが開けません。"ERROR_GYO_WARP);exit(1);
	}
