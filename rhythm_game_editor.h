...
void editor(char note_gamen[ARRAY_GYO][7], FILE* fp, char editor_mode, char name[80])
{
	...
	else
	{
		...
		while (fscanf(fp, "%d %d/%d %s", &editor.tempo[0], &editor.measure_beat, &editor.standard_beat, editor.note[0]) != EOF)
		{
			...
			editor_note_gyo_max++;
			if (editor_tempo_gyo_max != 0) editor_tempo_gyo_max--;	//BPM参照開始カウント
		}
		fclose(fp);
		fp = fp_work;
	}
	while (1)
	{
		...
		do	//選択メニュー1
		{
			...
			case 'g':
				...
				if (gyo_flg == 0x0&&editor_gyo>ARRAY_GYO-1)	//画面表示が最高列の時かつ、画面外への移動時 - 過去への移動 -
				{
					gyo_flg = 0x4;
				...
				{
					if (editor_note_gyo_max + editor_gyo == 0)		//最高列への移動
					{
						gyo_flg = 0x0;
		...
		{	
			...
			if (editor_tempo_gyo_max!=ARRAY_GYO*2-6) editor_tempo_gyo_max++;	//BPM参照開始カウント
			if(gyo_flg==0x0) editor_note_gyo_max++;	//ノーツデータ編集行の上限増加
		}
	}
	//↓編集終了時
	if (editor_note_gyo_max!=gamen_gyo_max)
	{
		...
		while (fscanf(fp_work, "%d %d/%d %s", &editor.tempo[13], &editor.measure_beat, &editor.standard_beat, &editor.note[0]) != EOF)
		{
			...
			gamen_gyo_max++;
			descent(gamen_gyo_max, editor_tempo_gyo_max, &editor, fp);
			if (editor_tempo_gyo_max != ARRAY_GYO * 2 - 4) editor_tempo_gyo_max++;	//BPM参照開始カウント
			for (j = 0;j < ARRAY_GYO;j++)
				printf("%d %s\n", editor.tempo[j], editor.note[j]);
			for (;j < ARRAY_GYO * 2 - 3;j++)
				printf("%d\n", editor.tempo[j]);
...
void data_hyouji(int gyo, int gyo_max, int tempo_gyo_max, MUSIC_DATA* data, char id)	//画面表示
{
	int i, j;
	int bpm_valie;
	int measure;
...
{ 
	...
	while (fscanf(fp_work, "%*d %*d/%*d %*s") != EOF)	//BPMがあるところまで移動
	{
		if (i == ARRAY_GYO - 4) break;
		else i++;
	}
	for (i=gyo;fscanf(fp_work, "%d %*d/%*d %*s", &bpm_work) != EOF&&i>=0;i--)	//BPMデータをコピー
	{
